...
public class CompressorScreen extends AbstractMachineScreen<CompressorMenu> {
    public CompressorScreen(CompressorMenu menu, Inventory playerInventory, Component title) {
        super(menu, playerInventory, title, ResourceLocation.fromNamespaceAndPath(Technocraft.MODID, "textures/gui/container/compressor.png"));
    }
    ...
    protected void renderProgress(GuiGraphics guiGraphics, int x, int y) {
        if (menu.isCompressing()) {
            int arrowWidth = 24;
            int progressScaled = (int) (arrowWidth * ((float) menu.getCompressProgress() / menu.getProcessTime()));
