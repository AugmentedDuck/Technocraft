...
/**
 * Crafting-table recipe for cutting a material into a cable.
 *
 * <p>The grid must contain exactly one {@link net.augmentedduck.technocraft.item.custom.CutterItem}
 * and exactly one stack matching {@link #input}, with nothing else. Unlike a
 * normal shapeless recipe, the cutter is not consumed: {@link #getRemainingItems}
 ...
 * would exceed max damage, an empty stack is returned instead and the cutter
 ...
 */
public class CutterItemRecipe implements CraftingRecipe {

    private final Ingredient input;
    private final ItemStack output;

    public CutterItemRecipe(Ingredient input, ItemStack output) {
        this.input = input;
        this.output = output;
    }
    ...
    public boolean matches(CraftingInput input, Level level) {
        ...
        for (int i = 0; i < input.size(); i++) {
            ...
            if (stack.getItem() == ModItems.CUTTER_ITEM.get()) {
                if (foundCutter) return false;
                foundCutter = true;
        ...
        return foundCutter && foundMaterial;
    }
    ...
    public NonNullList<Ingredient> getIngredients() {
        ...
        ingredients.add(Ingredient.of(ModItems.CUTTER_ITEM.get()));
        return ingredients;
    }
    ...
    public NonNullList<ItemStack> getRemainingItems(CraftingInput input) {
        ...
        for (int i = 0; i < input.size(); i++) {
            ItemStack stack = input.getItem(i);
            if (stack.getItem() != ModItems.CUTTER_ITEM.get()) continue;

            int newDamage = stack.getDamageValue() + 1;
            if (newDamage < stack.getMaxDamage()) {
                ItemStack cutter = stack.copy();
                cutter.setDamageValue(newDamage);
                remaining.set(i, cutter);
            }
        }
    ...
    public RecipeSerializer<?> getSerializer() {
        return ModRecipeSerializers.CUTTER_ITEM_SERIALIZER.get();
    }
